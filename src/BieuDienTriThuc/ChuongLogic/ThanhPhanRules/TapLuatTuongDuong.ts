...
export class TapLuat {
    ...
    public static LUAT_DE_MORGAN:number = 13;
    constructor() {
        this.tapLuat = [];
        this.xayDungTapLuat();
    }
    ...
    xayDungTapLuat() {
        ...
        this.tapLuat.push(
            new Luat(1,"Luat phep tuong duong", 
                    ...
                    boKiemTra(P: BieuThucMenhDe): LuatMessage | null {
                        ...
                        for (let i: number = 0; i < P.bieuThucCons.length; i++) {
                            if (P.bieuThucCons[i].toanTu.tenToanTu === ToanTu.PHU_DINH || 
                                (Helper.IS_BIEU_THUC_SO_CAP(P.bieuThucCons[i]) && P.bieuThucCons[i].id.includes(ToanTu.PHU_DINH+""))) {
                                builder.addBieuThucCon(P.bieuThucCons[i]);
                                // count++;
                            }
                        }
                    ...
                    ketQua(P: BieuThucMenhDe, con: LuatMessage): BieuThucMenhDe {
                        ...
                        for (let i: number = 0; i < P.bieuThucCons.length; i++) {
                            // if (Helper.IS_BIEU_THUC_SO_CAP(P.bieuThucCons[i])) {
                                ...
                                builder.addBieuThucCon(Helper.PHU_DINH_MENH_DE(P.bieuThucCons[i]));
                            // } else
            ...
            new Luat(14,
                ...
                new class implements DieuKien {
                    boKiemTra(P: BieuThucMenhDe): LuatMessage | null {
                        ...
                        let kt: boolean = false;
                        let back:BieuThucMenhDe = Helper.SAO_CHEP(P);
                        if (P.toanTu.tenToanTu !== ToanTu.PHU_DINH || Helper.IS_BIEU_THUC_SO_CAP(P))  return null;  
                        let tt: number = P.toanTu.tenToanTu === ToanTu.HOI ? ToanTu.TUYEN : ToanTu.HOI;
