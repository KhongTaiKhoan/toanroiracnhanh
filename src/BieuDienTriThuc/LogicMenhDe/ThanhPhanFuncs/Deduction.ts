...
import { DeducetiveRules, } from '../ThanhPhanRules/DeducetiveRules';
...
export class Deduction extends BaiTap{
    ...
    private GT: Expression[] = [];
    private KL: Expression;
    private manager:DeducetiveRules.ManagerRules = new DeducetiveRules.ManagerRules();
    private reasoning:Reasoning[] = [];
    private visited:Expression[] = []; /// MANG DANH DAU GT NAO DA DUOC DUYET;
    private helper = ExpressionHelper.Helper;
    private result:Reasoning[]=[];
    constructor(GT:string[],KL:string){
        super();
        GT.forEach(e => { this.GT.push(StringToExpression(e)) });
        this.KL = StringToExpression(KL);
    }
    
    giai(deBai?: string):Reasoning[] {
        this.setDafaultGT();
    ...
    private stringReasoning(re:Reasoning[]):string[]{
        let s:string[]=[];
        re.forEach(e=>{
            let str = `${e.id}. ${ExpressionToString(e.exp)} `;
    ...
    private deduce():boolean{
        while(true){
            /// DAU TIEN PHAI SAP XEP TAP GT
            this.sortGT();
           let listReasnoning:Reasoning[] =[];
        ...
        //    console.log('====END GT\n')
            ...
            for (let i = 0; i < this.GT.length; i++) {
                if (this.visited.findIndex(e => { return e.id === this.GT[i].id }) !== -1) continue;
                listReasnoning = this.manager.apply(this.GT[i], this.GT, this.visited);
                if (listReasnoning.length === 0) continue;
                for (let i = 0; i < listReasnoning.length; i++) {
                    this.addReasoning(listReasnoning[i]);
                    /// NEU LA LUAT RUT GON THI THEM VAO
                    if(i < listReasnoning.length-1 && listReasnoning[i].rule.id === DeducetiveRules.DeducetiveRule.LUAT_RUT_GON){
            ...
            if (listReasnoning.length === 0 )return false;
    ...
    private isStop() {
        for (let i = 0; i < this.GT.length; i++) {
            ...
            if(this.GT[i].id === this.KL.id) return true;   
            /// NEU GT TUONG DUONG KL
            let equivalence = new Equivalence()
                .giai(`${ExpressionToString(this.GT[i])}\u2261${ExpressionToString(this.KL)}`);
            if (equivalence !== null) {
                let arr = [];
                for (let j = 0; j < equivalence.length; j++) {
                    let r;
                    if (j === 0) r = new Reasoning(0, [this.GT[i]], equivalence[j].rule, equivalence[j].Exp());
                    else r = new Reasoning(0, [equivalence[j - 1].Exp()], equivalence[j].rule, equivalence[j].Exp());
        ...
        if (!this.helper.laTuDon(this.KL)) {
            ...
            let reas = new Reasoning(0,parent,new DeducetiveRules.LUAT_KET_HOP(),this.KL);
    ...
    private indentyReasoning(rs: Reasoning): Reasoning {
    ...
    private testing (){
        console.log('GT: ');
        this.GT.forEach(e=>{console.log(ExpressionToString(e))});
        console.log('KL: '+ExpressionToString(this.KL));
        console.log('-------------------');
        for (let i = 0; i <this.GT.length; i++) {
            let reasoning = this.manager.rules[0].apply(this.GT[i],this.GT,this.visited);
            if(reasoning.length!==0){
                reasoning.forEach(e=>{
                    console.log( `LUAT: ${e.rule.name} EXP: ${ExpressionToString(e.exp)} [${e.id}]`);
                });
                break;
            }            
        }
    ...
    private sortGT(){
        ...
        let arr:{index:number, score:number,exp:Expression}[] = [];
    ...
    private scoreGT(exp:Expression):number{
        ...
        let score = 0;
        score+= exp.operator.id !== Operts.Type.KEO_THEO ? S_SCORE:0;

        if (exp.operator.id === Operts.Type.PHU_DINH) exp = exp.childs[0];

        if (this.helper.laTuDon(this.KL)){
            for (let i = 0; i < exp.childs.length; i++) { 
                if (exp.childs[i].id === this.KL.id) score += SUB_EXPRESSION_SCORE;
         }
        }
        else
            for (let i = 0; i < this.KL.childs.length; i++) {
                let index = exp.childs.findIndex(e => { return e.id === this.KL.childs[i].id });
                if (index !== -1) score += SUB_EXPRESSION_SCORE;
            }
        return score;
    ...
    private tradingResult(crrReasoning:Reasoning):Reasoning{

       let crrRea_Id:number=0;
       let crrRea_IdParents:number[]=[];
       for (let i = 0; i < crrReasoning.expParent.length; i++) {
           ...
           /// con khong thi truy vet tiep
               ...
               let par:Reasoning = this.tradingResult(this.reasoning[index]);
...
export class Reasoning{
    ...
    private _rule: ExpressionRule.Rule;
    private _id: number;
    private _idParent: number[]=[];
    private _expParent: Expression[]; 
    
    constructor(id: number, expParent: Expression[], rule: ExpressionRule.Rule, exp: Expression) {
        this._id = id;
    ...
    public get exp(): Expression {
        return this._exp;
    }
    public set exp(value: Expression) {
        this._exp = value;
    }
    ...
    public get rule(): ExpressionRule.Rule {
        return this._rule;
    }
    public set rule(value: ExpressionRule.Rule) {
        this._rule = value;
    }
    public get expParent(): Expression[] {
        return this._expParent;
    }
    public set expParent(value: Expression[]) {
