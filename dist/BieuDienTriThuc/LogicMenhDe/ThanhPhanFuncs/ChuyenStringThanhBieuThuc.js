...
exports.StringToExpression = void 0;
...
//     }
// }
function StringToExpression(chuoi) {
    let cha = null;
    let hienTai = new Expression_1.Expression();
    let toanTu = new Operator_1.OperatorFactory().create(Operator_1.Operts.Type.NONE);
    for (let i = 0; i < chuoi.length; i++) {
        ...
        if (chuoi[i] === '\n' || chuoi[i] === '\r')
            ...
            let bt_con = Helper_1.ExpressionHelper.Helper.createConstant(Expression_1.Expression.TRUE);
            bt_con.parent = hienTai;
            hienTai.addChild(bt_con);
        }
        else if (chuoi[i] === '0') {
            let bt_con = Helper_1.ExpressionHelper.Helper.createConstant(Expression_1.Expression.FALSE);
            bt_con.parent = hienTai;
            hienTai.addChild(bt_con);
        }
        else if (chuoi[i] === ')') {
            while (true) {
                if (hienTai.parent !== null)
                    hienTai = hienTai.parent;
                cha = hienTai.parent;
                if (hienTai.operator.id !== Operator_1.Operts.Type.PHU_DINH)
                    break;
            }
        }
        else if (chuoi[i] === '(') {
            cha = hienTai;
            hienTai = new Expression_1.Expression();
            hienTai.parent = cha;
            cha.addChild(hienTai);
        }
        else if (!Operator_1.Operator.symbols.includes(chuoi[i])) {
            if (hienTai.operator.id !== Operator_1.Operts.Type.PHU_DINH) {
                let bt_con = Helper_1.ExpressionHelper.Helper.createPrime(chuoi[i]);
                bt_con.parent = hienTai;
                hienTai.addChild(bt_con);
            }
            else {
                hienTai.addChild(Helper_1.ExpressionHelper.Helper.createPrime(chuoi[i]));
                while (true) {
                    if (hienTai.parent !== null)
                        hienTai = hienTai.parent;
                    cha = hienTai.parent;
            ...
            toanTu = new Operator_1.OperatorFactory().create2(chuoi[i]);
            if (toanTu.id !== Operator_1.Operts.Type.PHU_DINH) {
                hienTai.operator = toanTu;
            }
            else if (toanTu.id === Operator_1.Operts.Type.PHU_DINH) {
                cha = hienTai;
                hienTai = new Expression_1.Expression();
                hienTai.parent = cha;
                hienTai.operator = toanTu;
    ...
    if ((hienTai.childs.length === 1 && hienTai.operator.id === Operator_1.Operts.Type.NONE))
        return hienTai.childs[0];
    return hienTai;
}
