    ...
    xayDungTapLuat() {
        ...
        this.tapLuat.push(new Luat_1.Luat(13, 'Luat De Morgan', new class {
            ...
            ketQua(P, con) {
                ...
                for (let i = 0; i < P.bieuThucCons.length; i++) {
                    if (Helper_1.Helper.IS_BIEU_THUC_SO_CAP(P.bieuThucCons[i])) {
                        builder.addBieuThucCon(Helper_1.Helper.BIEU_THUC_SO_CAP(P.bieuThucCons[i].id.split(ToanTuLogic_1.ToanTu.PHU_DINH + "")[1]));
                    }
                    else
        ...
        /// LUAT DE MORGAN
        this.tapLuat.push(new Luat_1.Luat(14, 'Luat DE Morgan', new class {
            boKiemTra(P) {
                let tapLuat = new TapLuat();
                let kt = true;
                /// KIEM TRA BIEU THUC CHA CO AP DUNG LUAT KHONG
                for (let i = 0; i < tapLuat.tapLuat.length; i++) {
                    if (P.cha !== null)
                        if (tapLuat.tapLuat[i].boKiemTra(P.cha) !== null) {
                            let id = tapLuat.tapLuat[i].id;
                            if (id > 2 && id <= 13) {
                                kt = false;
                                break;
                            }
