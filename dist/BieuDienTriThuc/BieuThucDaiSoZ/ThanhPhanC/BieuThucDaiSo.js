"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.KhonGianSoNguyen = void 0;
const Helper_1 = require("../ThanhPhanFuncs/Helper");
const HauTo_1 = require("../ThanhPhanFuncs/HauTo");
var KhonGianSoNguyen;
(function (KhonGianSoNguyen) {
    class BieuThuc {
        constructor() {
            this._id = '';
            this._operator = '';
            this._childs = [];
            this._value = null;
            this._kind = 2;
            this._parent = null;
            //#endregion
        }
        addChild(child) {
            this.childs.push(child);
        }
        insertChildAt(index, child) {
            this.childs[index] = child;
            this.id = this.refeshID();
        }
        refeshID() {
            //// ID duoc quy dinh nhu sau
            /// 1. NEU LA PHEP TRU HAY CHIA THI KHONG THAY DOI VI TRI
            /// 2. NEU LA PHEP CONG HAY NHAN SE SAP XEP LAI
            ////// + UU TIEN CAC HANG -> BIEN -> BIEU THUC PHUC HOP
            ////// + VOI HANG SE SAP XEP THEM THEO CHIEU TANG DAN
            ////// + VOI BIEN SE SAP XEP THEO CHIEU A->Z
            if (this.operator === '-' || this.operator === '/')
                return this.childs[0].id + ' ' + this.childs[1].id + ' ' + this.operator;
            let hangSo = [];
            for (let i = 0; i < this.childs.length; i++) {
                if (this.childs[i].kind === BieuThuc.HANG_SO)
                    hangSo.push(this.childs[i].value);
            }
            hangSo.sort((a, b) => {
                if (a > b)
                    return 1;
                if (a < b)
                    return -1;
                return 0;
            });
            let bienSo = [];
            for (let i = 0; i < this.childs.length; i++) {
                if (this.childs[i].kind === BieuThuc.BIEN_SO)
                    bienSo.push(this.childs[i].id);
            }
            bienSo.sort();
            let normal = [];
            for (let i = 0; i < this.childs.length; i++) {
                if (this.childs[i].kind === BieuThuc.BIEN_SO || this.childs[i].kind === BieuThuc.HANG_SO)
                    continue;
                normal.push(this.childs[i]);
            }
            let str_id = '';
            hangSo.forEach(e => {
                str_id += e + ' ';
            });
            bienSo.forEach(e => {
                str_id += e + ' ';
            });
            normal.forEach(e => {
                str_id += e.id + ' ';
            });
            str_id += this.operator;
            return str_id;
        }
        toString() {
            if (this.kind === BieuThuc.HANG_SO || this.kind === BieuThuc.BIEN_SO)
                return this.id;
            let left = this.childs[0];
            let right = this.childs[1];
            if (this.parent === null && this.operator === '*' && left.id === '-1' && right.kind === KhonGianSoNguyen.BieuThuc.BIEN_SO)
                return '-' + right.toString();
            if (this.operator === '*' && left.id === '-1')
                return right.toString();
            if (right.operator === '*' && right.childs[0].id === '-1' && this.operator === '+')
                return `${left.toString()} - ${right.toString()}`;
            if (right.operator === '*' && right.childs[0].id === '-1' && this.operator === '-')
                return `${left.toString()} + ${right.toString()}`;
            return `${left.toString()} ${this.operator} ${right.toString()}`;
        }
        replaceVariable(obj) {
            if (this.kind === BieuThuc.HANG_SO || this.kind === BieuThuc.BIEN_SO) {
                if (this.kind === BieuThuc.BIEN_SO) {
                    let index = obj.findIndex(e => { return e.var === this.id; });
                    if (index !== -1) {
                        if (HauTo_1.HauTo.isNumber(obj[index].value))
                            return Helper_1.HelperBieuThucDaiSo.Helper.TAO_HANG_SO(parseInt(obj[index].value));
                        else
                            return Helper_1.HelperBieuThucDaiSo.Helper.TAO_BIEN_SO(obj[index].value);
                    }
                    else
                        return BieuThuc.copy(this);
                }
                else
                    return BieuThuc.copy(this);
            }
            else {
                let newExpr = BieuThuc.copy(this);
                for (let i = 0; i < this.childs.length; i++) {
                    newExpr.childs[i] = this.childs[i].replaceVariable(obj);
                }
                newExpr.refeshID();
                return newExpr;
            }
            return new BieuThuc();
        }
        static copy(B) {
            if (B.kind === BieuThuc.HANG_SO)
                return Helper_1.HelperBieuThucDaiSo.Helper.TAO_HANG_SO(B.value);
            if (B.kind === BieuThuc.BIEN_SO)
                return Helper_1.HelperBieuThucDaiSo.Helper.TAO_BIEN_SO(B.id);
            let newB = new BieuThuc();
            newB.operator = B.operator;
            newB.kind = B.kind;
            for (let i = 0; i < B.childs.length; i++) {
                newB.childs.push(BieuThuc.copy(B.childs[i]));
            }
            return newB;
        }
        //#region  GETTER AND SETTER
        get parent() {
            return this._parent;
        }
        set parent(value) {
            this._parent = value;
        }
        get childs() {
            return this._childs;
        }
        set childs(value) {
            this._childs = value;
        }
        get operator() {
            return this._operator;
        }
        set operator(value) {
            this._operator = value;
        }
        get id() {
            if (this._id !== '')
                return this._id;
            else
                this.id = this.refeshID();
            return this._id;
        }
        set id(value) {
            this._id = value;
        }
        get value() {
            if (this._value === null)
                throw new Error('Chua cai dat value cho bien');
            if (this.kind !== BieuThuc.PHUC_HOP)
                return this._value;
            let left = 0;
            let right = 0;
            try {
                left = this.childs[0].value;
                right = this.childs[1].value;
            }
            catch (error) {
                throw new Error('Can cai dat gia tri cho bien');
            }
            switch (this.operator) {
                case '+':
                    return left + right;
                case '-':
                    return left - right;
                case '*':
                    return left * right;
                case '/':
                    return left / right;
            }
            return 0;
        }
        set value(value) {
            this._value = value;
        }
        get kind() {
            return this._kind;
        }
        set kind(value) {
            this._kind = value;
        }
    }
    BieuThuc.HANG_SO = 0;
    BieuThuc.BIEN_SO = 1;
    BieuThuc.PHUC_HOP = 2;
    KhonGianSoNguyen.BieuThuc = BieuThuc;
    class BieuThucBuilder {
        constructor() {
            this.expr = new BieuThuc();
        }
        addChild(child) {
            this.expr.childs.push(child);
            return this;
        }
        addOperator(operator) {
            this.expr.operator = operator;
            return this;
        }
        addParent(parent) {
            this.expr.parent = parent;
        }
        build() {
            let rs = this.expr;
            this.expr = new BieuThuc();
            return rs;
        }
    }
    KhonGianSoNguyen.BieuThucBuilder = BieuThucBuilder;
})(KhonGianSoNguyen = exports.KhonGianSoNguyen || (exports.KhonGianSoNguyen = {}));
