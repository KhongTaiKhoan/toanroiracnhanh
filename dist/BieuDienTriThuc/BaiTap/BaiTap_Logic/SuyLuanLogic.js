...
exports.LoiGiaiSuyDien = exports.SuyDien = void 0;
...
const TapSuyDien_1 = require("../../ChuongLogic/ThanhPhanRules/TapSuyDien");
const RutGonBieuThuc_1 = require("./RutGonBieuThuc");
const MenhDeTuongDuong_1 = require("./MenhDeTuongDuong");
const ChuyenStringThanhBieuThuc_1 = require("../../ChuongLogic/ThanhPhanFuncs/ChuyenStringThanhBieuThuc");
var SuyDien;
(function (SuyDien) {
    class SuyDienLoGic extends BaiTap_1.BaiTap {
        constructor() {
            super();
            this.giaThiet = new BieuThucMenhDe_1.BieuThucMenhDe();
            this.ghiNho = [];
            this.loiGiai = [];
            this.tapLuatSuyDien = new TapSuyDien_1.TapSuyDien(this);
        }
        giai() {
        ...
        lapGiaiQuyet() {
            let ketLuan = this.giaThiet.bieuThucCons[SuyDienLoGic.KET_LUAN];
            let giaThiet = this.giaThiet.bieuThucCons[SuyDienLoGic.GIA_THIET];
            ...
            while (true) {
                ...
                for (let i = 0; i < length; i++) {
                    if (Helper_1.Helper.IS_BIEU_THUC_SO_CAP(giaThiet.bieuThucCons[i]))
                        continue;
                    let rs = this.tapLuatSuyDien.suyDien(this.giaThiet, giaThiet.bieuThucCons[i]);
        ...
        buocSapXep() {
            ...
            for (let i = 0; i < giaThiet.bieuThucCons.length; i++) {
                //// CHUA MENH DE
                let menhDe = giaThiet.bieuThucCons[i];
                let diem = 0;
                if (Helper_1.Helper.IS_BIEU_THUC_SO_CAP(giaThiet.bieuThucCons[i])) {
                    mang.push({ diem: -1, viTri: i });
                    continue;
                }
                if (menhDe.toanTu.tenToanTu === ToanTuLogic_1.ToanTu.KEO_THEO) {
                    let index = giaThiet.bieuThucCons.findIndex(e => { return e.id === menhDe.bieuThucCons[0].id; });
                    if (index !== -1)
                        diem += MAX;
                    index = giaThiet.bieuThucCons.findIndex(e => { return e.id === menhDe.bieuThucCons[1].id; });
                    if (index !== -1)
                        diem += MAX;
                    let rg_1 = new RutGonBieuThuc_1.RutGonBieuThuc(menhDe.bieuThucCons[0]).giai();
                    let rg_2 = new RutGonBieuThuc_1.RutGonBieuThuc(menhDe.bieuThucCons[1]).giai();
                    for (let j = 0; j < giaThiet.bieuThucCons.length; j++) {
                        if (i == j)
                            continue;
                        let rg = new RutGonBieuThuc_1.RutGonBieuThuc(giaThiet.bieuThucCons[j]).giai();
                        if (rg_1.ketQua.id === ((_a = rg.ketQua) === null || _a === void 0 ? void 0 : _a.id) || rg_2.ketQua.id === rg.ketQua.id) {
                            diem += MAX;
                            break;
                        }
                    }
                ...
                let count = 0;
                for (let j = 0; j < ketLuan.bieuThucCons.length; j++) {
                    if (menhDe.id.includes(ketLuan.bieuThucCons[j].id))
                        count++;
                }
                if (count !== 0)
                    diem += count;
            ...
            for (let i = 0; i < mang.length; i++) {
                if (i === mang[i].viTri)
                    continue;
                if (duyetRoi.includes(i))
                    continue;
                let temp = giaThiet.bieuThucCons[mang[i].viTri];
        ...
        truyVet(bt) {
            let loiGiaiSuyDien = this.ghiNho.find(e => e.bieuThucKetQua.id === bt.id);
            if (loiGiaiSuyDien !== undefined)
                return loiGiaiSuyDien.index;
            return -1;
        }
        ...
        themLoiGiai(data, target) {
            ...
            if (this.soSanhKetQua(data.bieuThuc))
                return true;
            return false;
        }
        soSanhKetQua(P) {
            if (P.id === this.giaThiet.bieuThucCons[SuyDienLoGic.KET_LUAN].id)
                return true;
            ...
            if (detail !== null) {
                detail.forEach(e => {
                    this.themLoiGiai(new SuyDien.dataNotiFy(ChuyenStringThanhBieuThuc_1.ChuyenStringThanhBieuThuc.chuyenDoi(e.btKetQua), e.luat));
                });
                return true;
            }
            return false;
        ...
        ghiSuKien(P, target) {
            ...
            if (target !== undefined && target.length !== 0) {
                for (let i = 0; i < target.length; i++) {
                    ...
                    if (index !== -1) {
                        if (Helper_1.Helper.IS_BIEU_THUC_SO_CAP(giaThiet.bieuThucCons[index]))
                            continue;
    ...
    SuyDienLoGic.GIA_THIET = 0;
    SuyDienLoGic.KET_LUAN = 1;
    SuyDienLoGic.TIEP_TUC = 0;
    SuyDienLoGic.CHUAN_BI_KET_THUC = 1;
    SuyDienLoGic.KET_THUC = 2;
    SuyDien.SuyDienLoGic = SuyDienLoGic;
    class dataNotiFy {
        constructor(bieuThuc, tenLuat, target) {
    ...
    SuyDien.dataNotiFy = dataNotiFy;
})(SuyDien = exports.SuyDien || (exports.SuyDien = {}));
class LoiGiaiSuyDien {
    constructor(bieuThucKetQua, index, target, luat) {
...
exports.LoiGiaiSuyDien = LoiGiaiSuyDien;
